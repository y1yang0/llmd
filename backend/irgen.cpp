...
IRGenerator::IRGenerator(const char* mdFileName)
    : context(),
      builder(context),
...
bool IRGenerator::parseMarkdown(const char* fileName) {
...
void IRGenerator::emitVariable(const std::string& varName, int value) {
...
void IRGenerator::emitBinaryExpr(const std::string& varName,
...
void IRGenerator::emitPrint(const std::string& text) {
    ...
    // Give the format string a meaningless name since ExecutionEngine required
    // that any global values must have a name
    std::string formatName("fmt");
    formatName += std::to_string(rand());
    Value* formatValue = builder.CreateGlobalStringPtr(res[0], formatName);
...
void IRGenerator::emitReturn() { builder.CreateRetVoid(); }

void IRGenerator::emitIf(int constVal, const std::string& label) {
...
void IRGenerator::emitIf(const std::string& name, const std::string& label) {
...
void IRGenerator::emitIfImpl(Value* cond, const std::string& label) {
...
void IRGenerator::emiFunction(const std::string& funcName) {
    func =
        Function::Create(FunctionType::get(builder.getVoidTy(), false),
                         Function::ExternalLinkage, "_" + funcName, &theModule);
